...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o ${ORDERER_ADDRESS} --ordererTLSHostnameOverride orderer0.org${ORG}.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o ${ORDERER_ADDRESS} --ordererTLSHostnameOverride orderer0.org${ORG}.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
## Install chaincode on peer0.org3 and peer0.org2
...
## expect org3 to have approved and org2 not to
checkCommitReadiness 3 "\"Org3MSP\": true" "\"Org2MSP\": false"
checkCommitReadiness 2 "\"Org3MSP\": true" "\"Org2MSP\": false"
...
checkCommitReadiness 3 "\"Org3MSP\": true" "\"Org2MSP\": true"
checkCommitReadiness 2 "\"Org3MSP\": true" "\"Org2MSP\": true"
